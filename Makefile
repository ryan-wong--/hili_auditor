...
void* hili_db2_parse_flow_init(db2_parse_init_info_t* mitm_parse_init_info_ptr)
{
    ...
    strcpy(ptr->cli_mac, mitm_parse_init_info_ptr->cli_mac);
	strcpy(ptr->srv_mac, mitm_parse_init_info_ptr->srv_mac);
            ...
            while(log_length_left > 0) {
                ...
                if(col_type[i] == 4){
                    ...
                    if(parser_f_cursor[0] == 0x0c){
                        uint8_t dot[1] = {0x2e}, now_float[1] = {0}, now_f_cursor[1] = {0};
                ...
                switch(col_type[i]) {
                    ...
                    case 0x37: { 
                        //按照已知长度，隔一个读进 operation_handle，先偷懒
                        hili_send_module_send_log_add_bytes2(ptr->operation_handle, OPR_REPLY, fifo_cache_ptr,\
                            offset+ptr->qrydsc_len+HILI_DB2_HEAD_LEN+cur_loc, col_size[i], SPILT_NOT_LAST, SEND_NOT_IMMEDIATELY);
                        cur_loc += col_size[i];
                        log_length_left -= col_size[i];
                        i++;
                    }
                        break;
                    case 0x39: {
                        col_size[i] = parser_cursor[0];
                         //按照该长度，隔一个读进 operation_handle，先偷懒
                        hili_send_module_send_log_add_bytes2(ptr->operation_handle, OPR_REPLY, fifo_cache_ptr,\
                            offset+ptr->qrydsc_len+HILI_DB2_HEAD_LEN+cur_loc, col_size[i], SPILT_NOT_LAST, SEND_NOT_IMMEDIATELY);
                        cur_loc += col_size[i];
                        log_length_left -= col_size[i];
